...
def main():
    # Process sampled files and add conversation columns
    sampled_dir = Path(__file__).parent / 'sampled'
    output_dir = Path(__file__).parent / 'sampled_formatted'
    output_dir.mkdir(exist_ok=True)
    
    if not sampled_dir.exists():
        print(f"❌ Error: {sampled_dir} does not exist. Run sampler.py --extend first!")
        return
    
    for input_file in sampled_dir.glob('*_sampled.jsonl'):
        output_file = output_dir / input_file.name.replace('_sampled', '_formatted')
